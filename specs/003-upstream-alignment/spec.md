...
- **FR-004**: MemoryLint's apply step MUST verify, before writing, that each target file is unchanged since the audit using whole-file content (SHA256) verification, and MUST refuse to write and direct the user to re-audit when the file no longer matches. (The guarantee is file-level via the existing staleness gate, not a new per-edit-range anchor mechanism.)
