...
- Q: What installation approaches should be offered to the user? → A: Three approaches in priority order: (1) Recommended: `npx adg plugins add obra/superpowers --skill test-driven-development --skill verification-before-completion --skill brainstorming --skill systematic-debugging --skill receiving-code-review --skill finishing-a-development-branch --skill dispatching-parallel-agents --skill requesting-code-review --skill writing-plans --skill executing-plans --skill subagent-driven-development -g` (plugin bundle with selective skills, globally installed, matching the current plugin-install contract); (2) Alternative: `npx adg skills add obra/superpowers --skill test-driven-development --skill verification-before-completion --skill brainstorming --skill systematic-debugging --skill receiving-code-review --skill finishing-a-development-branch --skill dispatching-parallel-agents --skill requesting-code-review --skill writing-plans --skill executing-plans --skill subagent-driven-development --global -y` (global skills with selective scope); (3) Alternative: `npx adg skills add obra/superpowers --skill test-driven-development --skill verification-before-completion --skill brainstorming --skill systematic-debugging --skill receiving-code-review --skill finishing-a-development-branch --skill dispatching-parallel-agents --skill requesting-code-review --skill writing-plans --skill executing-plans --skill subagent-driven-development -y` (project-level skills with selective scope).
...
**Independent Test**: Can be fully tested by running `/speckit-superb-check` with one or more skills intentionally absent from all supported discovery roots and verifying that (a) the output includes `adg`-based installation guidance for each missing skill, (b) an interactive install prompt is offered, and (c) after user confirmation the skills are installed and re-detected.
...
- **FR-009**: System MUST present three installation approaches in priority order:
  - (1) Recommended: `npx adg plugins add obra/superpowers --skill test-driven-development --skill verification-before-completion --skill brainstorming --skill systematic-debugging --skill receiving-code-review --skill finishing-a-development-branch --skill dispatching-parallel-agents --skill requesting-code-review --skill writing-plans --skill executing-plans --skill subagent-driven-development -g` (plugin bundle with selective skills)
...
- **FR-013**: System MUST resolve skill availability through a canonical discovery layer that is independent of installation mechanism and supports both direct skill-root installs and plugin-provided skill directories.
- **FR-014**: System MUST expose discovery and install orchestration as separate helper responsibilities so downstream commands can depend on filesystem discovery without implicitly coupling to `adg`.

### Key Entities

- **Skill**: A superpowers discipline or practice represented as a directory containing a `SKILL.md` file, discoverable via direct skill roots or plugin-provided `skills/<name>/SKILL.md` directories in workspace or global scope.
...
- **Discovery Helper**: A read-only helper interface that resolves a named skill to a concrete `SKILL.md` path, source scope, and installation type without performing installation.
- **Ensure Helper**: An installation-oriented helper interface that checks prerequisites, prints guidance, and executes explicit `adg` installation only after user confirmation.
...
- `npx adg plugins add obra/superpowers --skill test-driven-development --skill verification-before-completion --skill brainstorming --skill systematic-debugging --skill receiving-code-review --skill finishing-a-development-branch --skill dispatching-parallel-agents --skill requesting-code-review --skill writing-plans --skill executing-plans --skill subagent-driven-development -g` installs only the specific 11 superpowers skills as a plugin bundle while keeping the install global and aligned with the current plugin-install contract.
